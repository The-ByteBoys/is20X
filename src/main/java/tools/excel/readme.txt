...
2. Call chooseDocument(path. Give the path of a xlsx file as the parameter. It will throw exceptions if the path is wrong or if its not a xlsx file.
3. Call getSheet(SheetIndex). There are 8 classes. The sheetIndex range from 0 included to 7 included. Will throw NullPointer exception if chooseDocumument() isnt called or the sheet is out of bounds.
